...
uniform bool u_EnableDirLights;
uniform bool u_EnablePointLights;
uniform bool u_EnableSpotLights;
...
void main(){
	...
	if (u_EnableDirLights)
		for(int i = 0; i < NR_DIR_LIGHTS; i++)
			o_Color += vec4(directional(u_DirLights[i], Ka, Kd, Ks, Ke), 0.0);

	if (u_EnablePointLights)
		for(int i = 0; i < NR_POINT_LIGHTS; i++)
			o_Color += vec4(point(u_PointLights[i], Ka, Kd, Ks, Ke), 0.0);

	if (u_EnableSpotLights)
		for(int i = 0; i < NR_SPOT_LIGHTS; i++)
			o_Color += vec4(spot(u_SpotLights[i], Ka, Kd, Ks, Ke), 0.0);
