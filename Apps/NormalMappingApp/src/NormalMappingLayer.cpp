...
static bool s_UsingNormalmap = true;

void NormalMappingLayer::OnAttach()
{
	// Load Shader
	s_Shader = CreateRef<Shader>("assets/shaders/BlinnPhongNormal.glsl");
...
void NormalMappingLayer::OnDetach()
{
	s_Shader = nullptr;
	s_CubeModel = nullptr;
	s_Camera = nullptr;
...
void NormalMappingLayer::OnUpdate()
{
	s_Camera->Update();
	s_Shader->Use();
	...
	{
		s_UsingNormalmap = !s_UsingNormalmap;
		s_Shader->SetBool("u_UsingNormalmap", s_UsingNormalmap);
	}
}

void NormalMappingLayer::OnEvent(Event& e)
{
	EventDispatcher dispatcher(e);
	dispatcher.Dispatch<WindowResizeEvent>(LUM_BIND_EVENT_FN(NormalMappingLayer::OnWindowResize));
}

bool NormalMappingLayer::OnWindowResize(WindowResizeEvent& e)
{
